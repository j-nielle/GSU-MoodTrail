...
export const actions = {
	...
	editUser: async ({ request, locals: { supabase } }) => {
		...
		if(userID == user.id){ // just in case
			...
			try {
				const { data, error: getUserByIdError } = await adminAuthClient.getUserById(userID);
				console.error(error);
				if (getUserByIdError) throw getUserByIdError;

				const { data, error: updateUserByIdError } = await adminAuthClient.updateUserById(userID, {
					email: newEmail,
