  ...
  <form class="flex flex-col" method="POST" action="?/editStudent" use:enhance>
    ...
    <div class="my-2">
      <FloatingLabelInput size="small" style="outlined" name="editFName" type="text" 
        value={prevFName} label="First Name" minlength="3" required />
    </div>

    <div class="my-2 space-x-4 flex justify-start">
      <div class="w-fit">
        <FloatingLabelInput size="small" style="outlined" name="editMName" type="text" 
          value={prevMName} label="Middle Initial" maxlength="1" required />
      </div>
      <div class="w-full">
        <FloatingLabelInput size="small" style="outlined" name="editLName" type="text" 
    ...
    <p class="text-sm my-2">Previous Year Level: <span class="font-semibold text-orange-500">{yearLvl[prevYrLvl]}</span></p>
