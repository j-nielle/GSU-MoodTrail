...
<Modal title="Add New Student" bind:open={open} size="xs" class="w-full">
  <form class="flex flex-col" method="POST" action="?/addStudent" use:enhance>
    ...
    <div class="my-2">
      <FloatingLabelInput size="small" style="outlined" name="addFName" type="text" 
        label="First Name" required />
    </div>

    <div class="my-2 space-x-4 flex justify-start">
      <div class="w-fit">
        <FloatingLabelInput size="small" style="outlined" name="addMName" type="text" 
          label="Middle Initial" maxlength="1" required />
      </div>
      <div class="w-full">
        <FloatingLabelInput size="small" style="outlined" name="addLName" type="text" 
